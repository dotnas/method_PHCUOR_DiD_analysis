---
title: "Evaluating the Impact on Facility Birth of PHCUOR Policy in Nigeria: A Difference-in-Differences
  Analysis"
author: "Nasir Umar"
date: "2025-06-05"
output:
...
- Data source: UNICEF Multiple Indicator Cluster Survey (MICS3 2007, MICS4 2011, MICS5 2016)
...
```{r Load and Prepare Data mics3_ch.sav}
# Read SPSS file
mics3_ch_raw1 <- read_sav("Data/mics3_ch.sav")
```


```{r View Data mics3_ch.sav, eval=FALSE}
...
```{r Select and Rename mics3_ch Data Var}
...
```{r Load and Prepare Data mics3_wm.sav}
# Read SPSS file
mics3_wm_raw1 <- read_sav("Data/mics3_wm.sav")
```

```{r View Data mics3_mw.sav, eval=FALSE}
# Checking variable labels and structure
label(mics3_wm_raw1)
...
```{r Select and Rename mics3_wm Data Var}
...
```{r Data Merging mics3_ch & mics3_wm}
...
```{r Check merged data, eval=FALSE}
...
```{r Load and Prepare mics4_ch.sav}

# Read SPSS file
mics4_ch_raw1 <- read_sav("Data/mics4_ch.sav")
```

```{r View Data mics4_ch.sav, eval=FALSE}
# Checking variable labels and structure
label(mics4_ch_raw1)
...
```{r Select and Rename mics4_ch Data Var}
...
```
```{r View and data checks, eval=FALSE}
# Initial data checks
...
```
```{r Final checks mics4_ch, eval=FALSE}
...
```{r Load and Prepare mics4_wm.sav}
# Read SPSS file
mics4_wm_raw1 <- read_sav("Data/mics4_wm.sav")
```
```{r View Data mics4_wm.sav, eval=FALSE}
...
```{r Select and Rename mics4_wm Data Var}
...
```{r Check final mics4_wm data, eval=FALSE}
...
```{r Data Merging mics4_ch & mics4_wm}
# Removing cluster_id (already in child data)
...
```
```{r Check merged mics4_ch-mics4_wmdata, eval=FALSE}
# Check merged data structure
...
```{r Load and Prepare Data mics5_ch.sav}
mics5_ch_raw1 <- read_sav("Data/mics5_ch.sav")
```

```{r View Data mics5_ch.sav, eval=FALSE}
# View variable labels and structure
label(mics5_ch_raw1)
...
```{r Select and Rename mics5_ch Variables}
...
```{r Load and Prepare Data mics5_wm.sav}
# Read SPSS file
mics5_wm_raw1 <- read_sav("Data/mics5_wm.sav")
```

```{r View Data mics5_mw.sav, eval=FALSE}
# View variable labels and structure
label(mics5_wm_raw1)
...
```{r Select and Rename mics5_wm Variables}
...
```{r Check mics5_wm data, eval=FALSE}
...
```{r Check merged mics5_ch & mics5_wm data structure}
# Removing cluster_id (already in child data)
...
```{r Check merged mics5_chwn data, eval=FALSE}
...
```{r final datasets for each wave}
...
```{r Check stacked data, eval=FALSE}
...
```{r Facility Birth Coverage across Mics}
...
```{r Setup data for further Analysis}
...
```{r Unweighted vs. weighted results}
...
```{r Plots Unweighted vs. weighted results}
...
This results shows the estimated coefficients (and their significance) from two DiD models (feols unweighted and svyglm survey weighted), along with various model fit statistics.

**DiD Estimate Unweighted**
...
**Interpretation:** 
In the unweighted fixed-effects linear probability model, the PHCUOR policy was associated with a small, non-significant decrease in the probability of facility births in early adopter states compared to control states after the policy implementation. This suggests no clear causal impact from this model.

**DiD Estimate Weighted (Survey-Weighted)**

**Estimate:** 
0.159 (on the log-odds scale)

**Significance:** 
Not statistically significant (p > 0.1).

**Interpretation:** 
In the survey-weighted quasibinomial model, the PHCUOR policy was associated with a positive log-odds ratio of 0.159. This implies an increase in the odds of facility birth for treated states in the post-period relative to controls. However, this effect is not statistically significant. 

Therefore, we cannot confidently conclude that the observed increase is a true effect of the policy, as it could be due to chance given the data's variability and survey design.

**Policy Relevance**

**Policy Effect:** 
While the weighted DiD estimate is positive (indicating an increase in facility births in treated states), it is not statistically significant. This means that based on this analysis, we cannot conclude that the PHCUOR policy caused a statistically significant increase in facility births.

**Determinants:**
Other factors like education and wealth are consistently and strongly associated with a higher probability of facility births, suggesting that socioeconomic improvements and educational attainment are critical pathways to increasing facility deliveries regardless of specific health policies like PHCUOR. Maternal age also plays a role.

**Note:** 
The non-significant policy effect raise questions about policy implementation, intensity, or whether the timeframe was sufficient to observe a large impact. 

It could also be due to other factors (e.g., education, wealth) are more dominant drivers.

**Implication for methods:**
The difference between the unweighted (negative) and weighted (positive) DiD estimates highlughts the importance of accounting for complex survey designs (sampling weights, clustering) in epidemiological research. Without proper weighting, conclusions can be substantially biased.
